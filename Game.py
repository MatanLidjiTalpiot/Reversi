...
class Game:
    def __init__(self, player1, player2, size=8):
        ...
        self.board[(3,
                    4)] = BLACK  # maybe need to make player class and then player.number
        ...
        try:
            if player1.get_disk() != FIRST_COLOR or player2.get_disk() != SECOND_COLOR:
                raise ValueError("not the right colors for disks")
        except Exception as e:
            print(str(e))
            print(
                "initializing colors: " + player1 + " is white, " + player2 + " is black")
    ...
    def to_flip(self, disk, coordinate):
        ...
        to_flip_right_down = self.to_flip_in_line(disk,
                                                  self.get_right_down(y, x))
    ...
    def to_flip_in_line(self, disk, line):  # line is a nparray of tuples (y,x)
        ...
        for square in line:  # TODO what happens if we run out of board
            if self.board[square] == -disk:  # if there is an empty disk
                ...
                square] == disk:  # if there is a disk in our color at the end
                return ret
    ...
    def get_legal_moves(self, disk):
        ...
        for row in range(self.size):
            for column in range(self.size):
                square = (row, column)
                if self.board[square] == 0 and not self.to_flip(disk,
                                                                square) == []:
    ...
    def get_winner_disk(self):
        ...
        """
        if self.is_board_full() or (
                        self.get_legal_moves(
                            BLACK) == [] and self.get_legal_moves(
                    WHITE) == []):
    ...
    def play_game(self, to_print=False):
        ...
        while not self.is_board_full():
            op = self.players[self.number_of_turns_attempted % 2].choose_move(
                self)

            if op[1] is None:
                if self.players[(
                            self.number_of_turns_attempted + 1) % 2].choose_move(
                    self)[
                    1] is \
                        None:
                    break
                ...
                self.do_move(
                    self.players[self.number_of_turns_attempted % 2].get_disk(),
                    op[1])

            if to_print:
                print("player, ",
                      self.players[self.number_of_turns_attempted % 2].name,
                      " played ", op[1])
                print(self.board)
...
if __name__ == '__main__':
    ...
    while game.get_number_of_turns() < 64:
        ...
        if not game.get_legal_moves(1) == []:
            ...
            print("###########")
            print(game.board)
            print("black: ", game.get_black_number(), "white: ",
                  game.get_white_number())
